...
def test_changing_leverage_recalculates_margin_on_open_position(
    ...
    """
    Перевіряємо, що зміна leverage на живій позиції перераховує margin.

    Платформа явно обіцяє в модалці leverage: "Changing the leverage will
    also affect all open positions and orders in the same market." Цей тест
    перевіряє виконання цієї обіцянки на практиці.

    Формула: при зміні leverage позиція (Size, Entry price) НЕ змінюється,
    але margin перераховується як margin_new = margin_old × (L_old / L_new).

    Сценарій (напрямок 1x → 50x — безпечніший, бо leverage зростає, margin
    зменшується, кошти звільняються):
    1. Pre-condition: позицій немає, leverage 50x.
    2. Встановлюємо leverage = 1.
    3. Відкриваємо Long на POSITION_SIZE_USDC → margin ≈ $200.
    4. Запам'ятовуємо margin_before.
    5. Змінюємо leverage на 50.
    6. Чекаємо перерахунку margin у таблиці позицій (race condition
       аналогічна netting — UI оновлюється не миттєво).
    7. Перевіряємо: margin_after ≈ margin_before / 50 (±10%).
    8. Cleanup у finally: закрити позицію + повернути leverage на 50x.

    Запас ±10% покриває округлення, природну зміну ціни BTC між кліками,
    і комісії.
    """
    page = authenticated_trading_page
    page.open()
    ...
    try:
        # Підготовка 1: встановити leverage = 1
        page.set_leverage(1)

        # Підготовка 2: відкрити Long при 1x → margin ≈ $200
        page.open_long_position(POSITION_SIZE_USDC)
        expect(page.positions_tab_with_one).to_be_visible(timeout=POSITION_TIMEOUT_MS)

        # Зчитуємо стартовий margin (при 1x)
        margin_before = page.get_long_position_margin()

        # Дія: змінити leverage на 50 (50x більше за поточний 1x).
        # Платформа має перерахувати margin живої позиції.
        page.set_leverage(50)

        # Чекаємо реального перерахунку margin у DOM.
        # Без цього метод повертає старе значення через race condition
        # (платформа ще не оновила DOM на момент читання).
        margin_after = page.wait_for_long_position_margin_change(
            from_value=margin_before
        )

        # Головна перевірка: margin зменшився у 50 разів (1x → 50x).
        # Формула: new_margin = old_margin × (old_leverage / new_leverage)
        leverage_ratio = 1 / 50
        expected_margin = margin_before * leverage_ratio
        tolerance = expected_margin * 0.1  # ±10%

        assert abs(margin_after - expected_margin) <= tolerance, (
            f"Margin after leverage change (1x→50x) is {margin_after}, "
            f"expected {expected_margin:.2f} ±{tolerance:.2f}. "
            f"Margin before: {margin_before}. "
            f"Difference: {abs(margin_after - expected_margin):.2f}."
        )
    finally:
        # Teardown: окремі try/except як у тестах A/C.
        # set_leverage(50) — safety net на випадок, якщо тест впав до
        # того, як leverage повернувся на default.
        try:
            page.close_position()
