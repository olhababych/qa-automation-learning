...
"""
Reduce Only тести в ISOLATED-режимі на SOLUSDC — дзеркало
test_isolated_reduce_only.py. Фікстура перемикає акаунт на Isolated,
...
def test_reduce_only_blocks_position_increase_isolated_sol(isolated_mode_sol: SolTradingPage):
    """Isolated SOL: Reduce Only блокує збільшення позиції."""
...
def test_reduce_only_does_not_flip_position_direction_isolated_sol(isolated_mode_sol: SolTradingPage):
    """Isolated SOL: Reduce Only + менший Short зменшує size, не перевертає."""
    page = isolated_mode_sol
    page.open_long_position(POSITION_SIZE_USDC)
    expect(page.positions_tab_with_one).to_be_visible(timeout=POSITION_TIMEOUT_MS)
...
def test_reduce_only_blocks_order_when_no_position_exists_isolated_sol(isolated_mode_sol: SolTradingPage):
    """Isolated SOL: Reduce Only ордер без позиції відхиляється."""
    page = isolated_mode_sol
