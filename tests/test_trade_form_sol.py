...
def test_buy_long_button_visible_by_default(
    ...
    expect(authenticated_sol_trading_page.buy_long_button).to_be_visible()
...
def test_action_button_changes_when_short_selected(
    ...
    authenticated_sol_trading_page.select_short()

    # Sell / Short з'явився, Buy / Long зник
    expect(authenticated_sol_trading_page.sell_short_button).to_be_visible()
    expect(authenticated_sol_trading_page.buy_long_button).not_to_be_visible()
...
def test_size_input_updates_btc_equivalent(
    ...
    authenticated_sol_trading_page.fill_size("100")
    # BTC equivalent з'явився і показує НЕ-нульове значення
    expect(authenticated_sol_trading_page.size_btc_equivalent).to_be_visible()
    expect(authenticated_sol_trading_page.size_btc_equivalent).not_to_have_text("~0 BTC")